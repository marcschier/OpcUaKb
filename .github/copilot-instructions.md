...
- **MCP Server** (`OpcUaKb.McpServer`): Custom MCP server with 17 tools (search_nodes, get_type_hierarchy, get_spec_summary, search_docs, search_docs_rag, count_nodes, list_specs, compare_versions, suggest_model, check_compliance, validate_nodeset, the profile-graph tools list_profile_groups/get_profile/query_profiles/check_profile_conformance, and the asynchronous mapping tools create_companion_projection/get_companion_projection). Uses `ModelContextProtocol` SDK with HTTP SSE (default) + stdio (`--stdio`) transports.
- **MCP security modes**: `MCP_AUTH_MODE` (default `apikey`) is an opt-in, non-breaking switch. `apikey` = anonymous reads + `mcp-access-key` for writes (unchanged). `entra` = Entra ID bearer auth via `Microsoft.AspNetCore.Authentication.JwtBearer` (Authority `https://login.microsoftonline.com/{tenant}/v2.0`, `ValidateAudience`/`ValidateIssuer` → no token passthrough) plus the SDK's `AddMcp()` RFC 9728 Protected Resource Metadata + `WWW-Authenticate` challenge; optional `MCP_REQUIRED_SCOPE` (`scp` claim) and `MCP_FRONTDOOR_ID` (`X-Azure-FDID` edge lock). Fails closed at startup if entra mode lacks tenant/client/resource-host. The public-nuget `Microsoft.ModelContextProtocol.HttpServer`/MISE/Graph AI Gateway paved path is internal-only and documented (not implemented) in `SECURITY.md`. The optional Front Door + WAF + Key Vault edge is a **standalone** `infra/edge.bicep` (not in `main.bicep`); `main.bicep` only wires the opt-in env vars.
- **Hosted Agent** (`OpcUaKb.HostedAgent`): Foundry Hosted Agent using Microsoft Agent Framework + Responses protocol. Connects **directly** to `OpcUaKb.McpServer` via `ModelContextProtocol.Client` (`HttpClientTransport` + `McpClient.CreateAsync` + `ListToolsAsync`) so each of the 17 MCP tools is exposed as a distinct `McpClientTool` (`AIFunction`). Deployed via `azd deploy` (after `azd ai agent init` + `azd provision`); replaces the legacy Bot Framework custom engine agent. **Does NOT use a Foundry Toolbox** — earlier `GetToolboxToolsAsync` approach wrapped tools into one opaque `McpTool` invisible to GPT-4o.
- **Live-server semantic projection**: `create_companion_projection` accepts a full live-server NodeSet2 using the existing inline / `blob:` / allow-listed URL modes, stages it privately, and queues durable analysis. A queue-scaled internal mapping worker searches all latest official companion models, combines deterministic structure matching with constrained GPT ranking over closed candidate IDs, and writes a generated instantiated NodeSet2 plus JSON/CSV mapping and reports. `get_companion_projection` returns progress and private artifact/download references. Core Server/type-system nodes are excluded; already companion-typed nodes are pass-through; multiple unrelated projections are allowed.
...
- NuGet: `Azure.Search.Documents` 11.8.0-beta.1, `Azure.AI.OpenAI` 2.9.0-beta.1, `ModelContextProtocol.AspNetCore` 1.4.1 (server), `Microsoft.AspNetCore.Authentication.JwtBearer` 10.0.10 (server — opt-in entra mode), `ModelContextProtocol` 1.2.0 (client — HostedAgent), `Microsoft.Agents.AI.Foundry.Hosting` 1.3.0-preview.260423.1 (HostedAgent), `Azure.AI.Projects` 2.1.0-beta.1 (HostedAgent)
